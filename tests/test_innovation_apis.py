...
# 指向当前运行的「当前代码」后端。默认 8080；若本机 8080 被旧进程占用，
# 可用环境变量 SAFEAGENT_TEST_BASE 覆盖为实际端口（如 http://localhost:8084）。
import os as _os2
BASE = _os2.environ.get("SAFEAGENT_TEST_BASE", "http://localhost:8080")
results = []
...
# ----------------------------------------------------------------------
# 8. 优化回路 / 反馈（P1-3 收敛：写端点默认关闭，只读保持可用）
# ----------------------------------------------------------------------
def test_optimization_feedback():
    r = requests.post(f"{BASE}/api/optimization/feedback",
                      json={"user_query": "测试", "feedback": "good", "rating": 5},
                      timeout=10)
    ok = r.status_code in (200, 501)
    record("优化回路-反馈提交(写端点默认关闭时 501)", ok, f"status={r.status_code}")

    r2 = requests.get(f"{BASE}/api/optimization/trend", timeout=5)
    ok2 = r2.status_code == 200
    record("优化回路-趋势查询", ok2, f"status={r2.status_code}")

    r3 = requests.get(f"{BASE}/api/optimization/summary", timeout=5)
    ok3 = r3.status_code == 200 and isinstance(r3.json(), dict)
    record("优化回路-只读汇总", ok3, f"status={r3.status_code}")
