...
| 指标 | 数值 |
|------|------|
| 检测流水线层数 | **5 层**（Unicode规范化 → 高级解码 → 规则/AI/向量检测 → LLM语义仲裁） |
| 攻击样例库 | **210 条**（160 攻击 + 50 正常），覆盖 4 大政企场景 |
| 对抗变异策略 | **10 种**（全角替换/同形异义/零宽字符/Base64/实体编码/多语言混合等） |
| 攻击类型覆盖 | **8 大类**（提示注入/越狱/命令注入/SQL注入/XSS/数据泄露/内容注入/数据投毒） |
